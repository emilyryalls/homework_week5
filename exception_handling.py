...
# traceback     module that provides utilities for working with tracebacks.
# traceback     is a report generated by Python when an exception occurs which provides detailed info about:
#               1) The sequence of function calls that led to the error (call stack)
#               2) The line number where the error occurred.
#               3) The type of exception adn its associated message.
# crucial for debugging because it helps us pinpoint the source of errors.

# print_tb      is a function from the traceback module
#               Purpose: prints the traceback information (call stack) to the console.

names_tuple = 'Rod', 'Jane', 'Freddy'

#   TRY Block
# Try block attempts code tha might raise an exception
try:
    print("######## TRY ########")
